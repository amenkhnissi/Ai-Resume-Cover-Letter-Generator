...
FORBIDDEN = re.compile(r"(password|ssn|social\s*security|credit\s*card|api\s*key)", re.I)
...
async def generate_all(req: GenerateRequest):
    ...
    if len(resume) < SAFE_MIN or len(jd) < SAFE_MIN:
      raise HTTPException(400, detail="Resume and JD must be at least 50 chars.")
